...
pub struct Control201Gender {
    pub masc: String,
    pub femme: String,
    pub neut: String,
}

impl Control201Gender {
    pub fn parse(header: &Header, mut reader: &mut Cursor<&[u8]>) -> Result<Self> {
        ...
        let (masc_size, masc) = read_string(header, &mut reader)?;
        let (femme_size, femme) = read_string(header, &mut reader)?;
        let (neut_size, neut) = read_string(header, &mut reader)?;

        if masc_size + femme_size + neut_size != len {
            Err(anyhow::anyhow!("Gender fields don't total param size"))
        }
        else {
            Ok(Control201Gender { masc, femme, neut })
        }
    }

    pub fn write(&self, header: &Header, mut writer: &mut dyn Write) -> Result<()> {
        let mut encoded_strs = Vec::with_capacity(3);
        let (masc_size, masc_bytes) = get_string_bytes(header, &self.masc)?;
        encoded_strs.push(masc_bytes);
        let (femme_size, femme_bytes) = get_string_bytes(header, &self.femme)?;
        encoded_strs.push(femme_bytes);
        let (neut_size, neut_bytes) = get_string_bytes(header, &self.neut)?;
        encoded_strs.push(neut_bytes);

        let len = masc_size + femme_size + neut_size;
